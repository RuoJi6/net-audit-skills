...
- 仅有 `bin/` 目录下的 `.dll` 而无 `.cs` 源码 → **闭源项目，必须走共享二进制预处理流程**：先复用/生成 `{project}_audit/deobfuscated/` 与 `{project}_audit/decompiled/` 产物声明，再分析反编译代码（见 `references/DECOMPILE_STRATEGY.md`）
...
- 闭源项目：优先复用 `{project}_audit/decompiled/manifest.json` 声明的共享反编译产物；不存在或不完整时，按 `references/DECOMPILE_STRATEGY.md` 先反混淆/反编译并生成 manifest，再全局搜索 `namespace *.Controllers` 定位控制层
...
## 4. 共享二进制预处理流程（无 .cs 源码时）

完整流程详见 `references/DECOMPILE_STRATEGY.md`，要点：

1. **源码优先**：若项目存在可用 `.cs` / `.csproj` / `Program.cs` / `Startup.cs` / `App_Start` 源码，直接分析源码；仅在源码缺失或后端类只存在于 dll 时进入二进制预处理。
2. **先看共享反编译声明**：若 `{project}_audit/decompiled/manifest.json` 存在、状态为 `success` 或可接受的 `partial`，且 `input_dir` 与当前目标匹配，直接复用 `{project}_audit/decompiled/`。
3. **混淆识别（前置）**：先抽样查看 `bin/` 中目标 dll，若出现 `\u0001` / `\u0002` 之类成员名、单字符命名空间、或 `strings <dll> | grep -i confuser` 命中混淆器水印，说明已被混淆 → 跳到第 4 步；否则跳到第 5 步
4. **去混淆（仅当混淆时）**：用 `de4dot` 整批清洗 `bin/` → `{project}_audit/deobfuscated/bin_clean/`，并生成 `{project}_audit/deobfuscated/DEOBFUSCATE_MANIFEST.md` 与 `{project}_audit/deobfuscated/manifest.json`；后续 ilspycmd 必须优先使用该 `bin_clean/` 作为输入和 `--referencepath`
5. **反编译并声明**：用 **`ilspycmd`** 批量反编译 `bin/`（或 `{project}_audit/deobfuscated/bin_clean/`）下全部项目 dll，输出到 `{project}_audit/decompiled/<AssemblyName>/`，并生成 `{project}_audit/decompiled/DECOMPILE_MANIFEST.md` 与 `{project}_audit/decompiled/manifest.json`
6. **失败不静默忽略**：manifest 中必须记录每个 dll/exe 的 `success` / `partial` / `failed` / `skipped` 状态；若入口程序集失败，路由输出必须标注审计不完整
7. 定位入口：
   - 全局搜索 `RegisterRoutes` / `Application_Start` / `MapPageRoute` / `MapControllerRoute`
   - 全局搜索 `namespace *.Controllers`，过滤 `System.*`
8. 仅有 `Global.asax` 而无路由配置类时，先在共享反编译产物中确认 `Global.asax` 对应 dll 的 `Application_Start`，再定位其调用的 `RouteConfig` 类
9. 闭源 Web Forms：用 `.aspx` 中的 `Inherits` 值反查 dll → 类 → 方法
10. 反混淆与反编译产物都是项目级共享审计证据，供 `net-route-mapper`、`net-auth-audit`、`net-sql-audit` 等 skill 复用
...
- [ ] 闭源项目已生成/复用 `{project}_audit/deobfuscated/manifest.json`（未混淆时在反编译 manifest 中声明跳过原因）
- [ ] 闭源项目已生成/复用 `{project}_audit/decompiled/manifest.json`
- [ ] 闭源 dll/exe 已全部反编译并入库，失败或跳过程序集已记录
